...
class Perceiver(nn.Module):
    def __init__(
        ...
        self.instance_norm = nn.InstanceNorm1d(self.num_features, affine=True)
        
        # Patch embedding
        self.patch_embed = PatchEmbedding(self.seq_size, patch_size, self.num_features, hidden_dim, dropout)
    ...
    def forward(self, x):
        ...
        if x.shape[1] != self.seq_size:
            raise ValueError(f"Expected seq_len={self.seq_size}, got {x.shape[1]}")
        if x.shape[2] != self.num_features:
            raise ValueError(f"Expected num_features={self.num_features}, got {x.shape[2]}")
        x = x.float()
